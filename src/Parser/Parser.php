...
class Parser
{
    ...
    private function expr(): MathNode
    {
        ...
        try {
            $left = $this->term();
            while (($type = $this->cur()->getType()) === Token::PLUS || $type === Token::MINUS) {
                $tok = $this->eat();
                $right = $this->term();
                $left = $this->binaryNode($left, $tok->getType(), $right, $left->getStartPos(), $right->getEndPos());
            }
            return $left;
        } finally { --$this->depth; }
    }
    ...
    private function term(): MathNode
    {
        ...
        try {
            $left = $this->factor();
            while (($type = $this->cur()->getType()) === Token::MULTIPLY || $type === Token::DIVIDE) {
                $tok = $this->eat();
                $right = $this->factor();
                $left = $this->binaryNode($left, $tok->getType(), $right, $left->getStartPos(), $right->getEndPos());
            }
    ...
    private function primary(): MathNode
    {
        ...
        try {
            ...
            if ($type === Token::IDENTIFIER) {
                $this->eat();
                return new VariableNode($t->getValue(), null, $t->getStart(), $t->getEnd());
            }
            if ($type === Token::SQRT) {
                $s = $t->getStart();
                $this->eat();
            ...
            if ($type === Token::RADICAL) {
                ...
                $arg = $this->expr();
                $rp = $this->expect(Token::RPAREN);
                return new RootNode($deg, $arg, $s, $rp->getEnd());
            }
    ...
    {
        return $this->tokens[$this->pos] ?? new Token(Token::EOF, '', strlen($this->src), strlen($this->src));
    }
    ...
    private function expect(string $type): Token
    {
        ...
        if ($cur->getType() !== $type) {
            ...
            throw new MathParseException("Expected '{$type}', got '{$got}' at position {$cur->getStart()}.");
        }
        return $this->eat();
    }
    ...
    private function isImplicitFactor(): bool
    {
        return in_array($this->cur()->getType(), [
            Token::NUMBER, Token::IDENTIFIER, Token::PI,
            Token::LPAREN, Token::SQRT, Token::RADICAL
        ], true);
    }

    private function binaryNode(MathNode $left, string $opType, MathNode $right, int $start, int $end): MathNode
    {
        switch ($opType) {
            ...
            default: throw new \InvalidArgumentException("Unknown binary operator: {$opType}");
        }
    }
}